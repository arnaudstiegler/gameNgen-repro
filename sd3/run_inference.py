...
def run_inference():
    ...
    with torch.no_grad():
        ...
        prev_frames = []
        for i in range(10):
            original_image = torch.randn((1, 3, height, width)).to(device)
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            ...
            factor = latent_model_input.shape[1] // noise_pred.shape[1]
            noise_pred = noise_pred.repeat(1, factor, 1, 1)
            latents = noise_scheduler.step(
                noise_pred, t, latents, generator=None, return_dict=False
            )[0]
            # paper does not make it clear regarding dealing with the stacked latent whe denoising
            # for simplicity, we just loop over all latents and denoise them
            total_latents = latents.shape[1]
            each_latent_size = latents.shape[1] // factor
            for l_i in range(each_latent_size, total_latents, each_latent_size):
                latent = latents[:, l_i - each_latent_size:l_i, ...]
                image = vae.decode(
                ...
                image[0].save(f"inference_test/image_steps_{l_i}_{i}.png")
